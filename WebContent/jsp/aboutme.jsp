...
<%@page import="com.icode.chengcheng.po.News"%>
...
function deleteMessage(id){
	...
	location="/chengcheng/DeleteNewsServlet?nid="+id;
	}
}
</script>
</head>
...
<div class="templatemo_sidebar_wrapper float_l">
    ...
    <span> <img src="/chengcheng/upload/images/photo/${sessionScope.user.uphoto}" width="150" height="158" /><img src="/chengcheng/images/xrw.png" width="27" height="29" /> ${sessionScope.user.uname}</span>
        ...
        <div id="content_middle">
          ...
          for(News news:lstNews){
            ...
            <% if(type==1){	
          	  ...
          	  %>
         <div>提醒：<a href="/chengcheng/ShowInOthersServlet?uccid=<%=uccid%>"> <img src="/chengcheng/upload/images/photo/<%=uphoto%>"  alt="" width="50" height="50" align="middle"/></a><%=uname %>于<%=news.getNdate() %>参加了您参加的<a href="/chengcheng/ToCommentServlet?aid=<%=aid%>"><%=aname %></a>活动</div>  
                    </td>
           <td width="50">
         ...
         String uname = content[1];
     	 ...
     	 int aid = Integer.parseInt(content[3]);
     	 String aname  = content[4]; %>
         <div>提醒： <a href="/chengcheng/ShowInOthersServlet?uccid=<%=uccid%>"> <img src="/chengcheng/upload/images/photo/<%=uphoto%>"  alt="" width="50" height="50" align="middle"/></a><%=uname %>于<%=news.getNdate()%>在您发布的<a href="/chengcheng/ToCommentServlet?aid=<%=aid%>"><%=aname %></a>活动下留了新留言</div>  
                   </td>
           <td width="50">
           <input type="button" name="delete" id="delete" value="删除消息" onclick="javascript:deleteMessage('<%=news.getNid()%>')">
          ...
          <%}else if(type==3) {
        	...
        	int aid = Integer.parseInt(content[3]);
        	String aname  = content[4];%>
          <div>提醒： <a href="/chengcheng/ShowInOthersServlet?uccid=<%=uccid%>"> <img src="/chengcheng/upload/images/photo/<%=uphoto%>"  alt="" width="50" height="50" align="middle"/></a><%=uname %>于<%=news.getNdate() %>回复了您在<a href="/chengcheng/ToCommentServlet?aid=<%=aid%>"><%=aname %></a>活动中的留言</div>  
                     </td>
           <td width="50">
           <input type="button" name="delete" id="delete" value="删除消息" onclick="javascript:deleteMessage('<%=news.getNid()%>')">
          <%}else if(type==0){
     ...
     <div>好友： <a href="/chengcheng/ShowInOthersServlet?uccid=<%=news.getNfrom()%>"> <img src="/chengcheng/upload/images/photo/<%=uphoto%>"  alt="" width="50" height="50" align="middle"/></a><%=uname %>于<%=news.getNdate() %>悄悄告诉你：<%=ncontent %></div>  
                       </td>
           <td width="50">
           <input type="button" name="delete" id="delete" value="删除消息" onclick="javascript:deleteMessage('<%=news.getNid()%>')">
          <%}else if(type==4){ 
        	  ...
        	  String uname = content[0];
        	  String uphoto = content[1];
           %><div>好友： <a href="/chengcheng/ShowInOthersServlet?uccid=<%=news.getNfrom()%>"> <img src="/chengcheng/upload/images/photo/<%=uphoto%>"  alt="" width="50" height="50" align="middle"/></a><%=uname %>于<%=news.getNdate() %>想要加您为好友 </div> 
            </td><td width="50">
            <input type="button" id="friend" name="friend" onclick="location = '/chengcheng/AddFriendsServlet?uccid=<%=news.getNfrom() %>&type=0';" value="同意">
            <input type="button" id="friend2" name="friend2" onclick="location = '/chengcheng/AddFriendsServlet?uccid=<%=news.getNfrom() %>&type=1';" value="不同意">
            
           <%}else if(type==5){ 
        	  String[] content = news.getNcontent().split("/");
         	  String uname = content[0];
         	  String uphoto = content[1];
            %><div>好友： <a href="/chengcheng/ShowInOthersServlet?uccid=<%=news.getNfrom()%>"> <img src="/chengcheng/upload/images/photo/<%=uphoto%>"  alt="" width="50" height="50" align="middle"/></a><%=uname %>于<%=news.getNdate() %>同意加您为好友</div>  
        	           </td>
           <td width="50">
           <input type="button" name="delete" id="delete" value="删除消息" onclick="javascript:deleteMessage('<%=news.getNid()%>')">
        	<%}else if(type==6){ 
        		  String[] content = news.getNcontent().split("/");
             	  ...
             	  String uphoto = content[1];
                %>
              <div>好友： <a href="/chengcheng/ShowInOthersServlet?uccid=<%=news.getNfrom()%>"> <img src="/chengcheng/upload/images/photo/<%=uphoto%>"  alt="" width="50" height="50" align="middle"/></a><%=uname %>于<%=news.getNdate() %>拒绝加您为好友</div>  
                       </td>
           <td width="50">
           <input type="button" name="delete" id="delete" value="删除消息" onclick="javascript:deleteMessage('<%=news.getNid()%>')">
            <%} %>
