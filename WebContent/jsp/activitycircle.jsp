      ...
      <div id="templatemo_content">
       <div id="content_top"></div>
        <div id="content_middle">
          <h3>活动圈</h3>
          <form id="form5" name="form5" method="post" action="ShowInActivityCircleServlet?type=5">
          <tr><td width="100" height="30">您的爱好：</td>
          <input type="button" name="btnSelelectAll" id="btnSelelectAll" value="全选" onClick="javascript:selectAllCheckBox('checkbox');" onMouseOver="this.style.borderColor='#75cd02'" onMouseOut="this.style.borderColor='#dcdcdc'" class="btn1 pbtn1" />
  		            ...
  		            <input type="submit" class="btn4" name="button1" id="button1" value="找活动" /></tr>
          <tr><td> <p>&nbsp;
            </p>
              <p>
                <%
          List<Tags> lstActHobby = (List<Tags>) request.getAttribute("lstActHobby");
           List<Integer> lstTid = (List<Integer>) request.getAttribute("lstTid");
          if(lstActHobby!=null){    	  
          for(Tags tag:lstActHobby){
          %>   
                <%
          if(lstTid!=null&&lstTid.contains(tag.getTid())){
          %>
                <label>
                  <input type="checkbox" name="checkbox" id="checkbox<%=tag.getTid()%>" value="<%=tag.getTid()%>" checked=true/>
                </label>
                <%=tag.getTname()%><%
          }else { %>
                <label>
                  <input type="checkbox" name="checkbox" id="checkbox<%=tag.getTid()%>" value="<%=tag.getTid()%>"/>
                </label>
                <%=tag.getTname()%><%} 
          }
          ...
          <tr>
          <td width="79" >
          <form id="form1" name="form1" method="post" action="ShowInActivityCircleServlet?type=1">
            <label><input type="submit"  name="button1" id="button1" class="btn5" value="时间" onMouseOver="this.style.backgroundPosition='left -25px'" onMouseOut="this.style.backgroundPosition='left top'" /></label>
          </form></td>
          <td width="79" >
         <form id="form2" name="form2" method="post" action="ShowInActivityCircleServlet?type=2">
          <label><input type="submit"  name="button2" id="button2" class="btn5" value="热度" onMouseOver="this.style.backgroundPosition='left -25px'" onMouseOut="this.style.backgroundPosition='left top'" /></label>
         </form>
         </td>
         <td width="79">
         <form id="form3" name="form3" method="post" action="ShowInActivityCircleServlet?type=3">
          <label><input type="submit"  name="button3" id="button3" class="btn5" value="讨论量" onMouseOver="this.style.backgroundPosition='left -25px'" onMouseOut="this.style.backgroundPosition='left top'" /></label>
         </form>
         </td>
         <td width="79" >
         <form id="form4" name="form4" method="post" action="ShowInActivityCircleServlet?type=4">
          <label><input type="submit"  name="button4" id="button4" class="btn5" value="参与人数" onMouseOver="this.style.backgroundPosition='left -25px'" onMouseOut="this.style.backgroundPosition='left top'" /></label>
          </form>
          </td>
          </tr>
          </table>
          ...
          <table width="720" border="1" style="border-style:none;">
            <tr>
              <td>            
                <div><a href="/chengcheng/ShowInOthersServlet?uccid=<%=act.getOwner_id()%>"><img src="/chengcheng/upload/images/photo/<%=act.getUphoto()%>" width="50" height="50"  alt=""/></a><span><%=act.getUname()%>发布了：</span></div>           
         <div>
          <table width="603" border="0" align="right">
             ...
             <tr align="right">
              ...
              <input type="submit" name="text<%=act.getAid()%><%=act.getTid()%>" id="text<%=act.getAid()%><%=act.getTid()%>" value="查看详细内容" onClick="javascript:showContent('text<%=act.getAid()%><%=act.getTid()%>','content<%=act.getAid()%><%=act.getTid()%>');" onMouseOver="this.style.borderColor='#86c6f7'" onMouseOut="this.style.borderColor='#dcdcdc'" class="btn3 pbtn1" /><br />            
              </td>
             </tr>
              <tr align="right">
               <td><div id="content<%=act.getAid()%><%=act.getTid() %>" style="display:none"><p><%=act.getAcontent()%></p>
                   <p> <input type="submit" name="text" id="text" value="返回" onClick="javascript:hideContent('text<%=act.getAid()%><%=act.getTid() %>','content<%=act.getAid()%><%=act.getTid()%>');" onMouseOver="this.style.borderColor='#86c6f7'" onMouseOut="this.style.borderColor='#dcdcdc'" class="btn3 pbtn1" /><br /></p> 
                        </div>
               </td>
             </tr>
           </table>
